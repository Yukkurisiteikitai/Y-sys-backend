...
```"""
    ...
    mbti_prompt = """
# MBTI診断プロンプト：コミュニケーションスタイル分析

あなたは優れたMBTI分析の専門家です。私が提供する会話履歴、SNS投稿、またはコミュニティでの発言から、その人物のMBTIタイプを分析してください。一貫性のあるパターンを見つけ、正確な診断を行ってください。

## 分析のための情報提供

以下の情報を提供します：
1. 会話履歴（DM、グループチャットなど）
2. Twitter/SNS投稿 
3. コミュニティ（フォーラム、ブログなど）での発言

※会話や投稿を以下にコピー＆ペーストします：
```
（ここにテキストを貼り付けてください）
```
...
それぞれの指標について、明確な根拠を示しながら分析してください：
...
1. **初期印象と全体的な傾向**：コミュニケーションスタイルの主な特徴
2. **各指標の分析**：E/I、S/N、T/F、J/Pそれぞれについて具体的な根拠を示す
3. **MBTIタイプの判定**：最も可能性の高いMBTIタイプとその理由
4. **代替の可能性**：次に可能性のあるタイプ（境界線上の場合）
5. **発達度の考察**：認知機能の発達状況と使用パターン

## 注意事項

- テキストだけでは完全に正確な診断はできないため、最も可能性の高い分析結果として提示します
- ステレオタイプに頼らず、認知機能の実際の使用パターンに基づいて判断します
- 文脈や状況による変動も考慮します
- 単一の発言ではなく、一貫したパターンを重視します
