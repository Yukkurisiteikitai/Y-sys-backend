...
from api.schemas import (
    SessionRequest, SessionResponse, MessageRequest, AbstractRecognitionResult,
    Episode, ConcreteUnderstandingResult, ThoughtProcess, FinalResponseData, FinalResponse
)
from dependencies import get_concrete_process, get_response_gen
...
async def get_session_messages(session_id: str, limit: int = 50, offset: int = 0, include_metadata: bool = False):
    if session_id not in sessions:
        raise HTTPException(status_code=404, detail="SESSION_NOT_FOUND")
    dummy_messages = [
        {"message_id": str(uuid.uuid4()), "role": "user", "content": "こんにちは", "timestamp": datetime.now().isoformat()},
        {"message_id": str(uuid.uuid4()), "role": "assistant", "content": "こんにちは！何かお話ししましょう。", "timestamp": datetime.now().isoformat()}
    ]
    return {
        "session_id": session_id,
        "messages": dummy_messages[offset:offset+limit],
        "pagination": {"total": len(dummy_messages), "limit": limit, "offset": offset}
    }
